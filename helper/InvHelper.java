...
public class InvHelper {
	...
	public static boolean sameMod(final ItemStack a, final ItemStack b) {
		return a != null && b != null
				&& (canStack(a, b) || a.getItem() == b.getItem() || getModID(a).equals(getModID(b)));
	}
	...
	public static ModContainer getModForItemStack(final ItemStack stack) {
		...
		for (final Map.Entry<String, ModContainer> entry : modList.entrySet()) {
			...
			for (int i = 0; i < modClassParts.length && i < itemClassParts.length && itemClassParts[i] != null
					&& itemClassParts[i].equals(modClassParts[i]); ++i) {
				++packageMatches;
			}
